...
//                                                                           |            |
//                                                                           v            |
// 服务端流程 [创建 socket] -> [绑定地址] -> [监听连接] -> [创建子进程] -> [接收数据] -> [发送数据] -> [关闭连接] -> [退出]
//                                                                           |                          ^
//                                                                           |                          |
//                                                                           +------------if(Q)---------+
...
private:
    ...
    void business_process() {
        ...
        // 3. 关闭连接, 子进程会各复制一份 sock_server 和 sock_client, 需要关闭
        close(socket_clnt);
        close(socket_serv);
...
// A: 每个进程分配的 socket 标识符应该是独立的
//    导致每次调用 accpet 都返回相同的值(因为 close 了)
//    但是在操作系统层面实际上是不一样的